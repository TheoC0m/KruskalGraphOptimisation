...
public class Entreprise {
	...
	public Entreprise(GraphMatrix g1) {
		...
		// nouveau graphe vide de la meme taille que g1
		this.reseau_entreprise = new GraphMatrix(g1.getTaille());
	}

	/*
	 * ajoute une ville (un noeud) au reseau de l'entreprise
	 */
	...
	/*
	 * ajoute une voie ferree (un arc) au reseau de l'entreprise
	 */
	...
	/*
	 * supprimme une voie ferree (un arc) du reseau de l'entreprise
	 */
	...
	/*
	 * optimise le reseau de l'entreprise en appliquant l'algorithme de kruskal
	 * sur le reseau national et celui de l'entreprise
	 */
	public void optimal() {

		// tant que reseau_entreprise a moins de taille-1 arcs et
		// reseau_nationnal a au moins un arc
		while (this.reseau_entreprise.numEdge() <= this.reseau_nationnal.getTaille() - 1
			...
			// si le reseau entreprise ne contient pas la ville lightest_edge[0]
			// issue de l'arc le plus leger
			if (!this.reseau_entreprise.getNodes().contains(lightest_edge[0])) {
				// on l'y ajoute
				this.add_city(lightest_edge[0]);
			}
			// si le reseau entreprise ne contient pas la ville lightest_edge[1]
			// issue de l'arc le plus leger
